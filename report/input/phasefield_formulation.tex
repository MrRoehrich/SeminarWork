...
\end{equation}
The energy balance with $e$ describing the internal energy per unit mass density reads
\begin{equation} \label{eq:energy_balance}
	\rho_{0}\dot{e}=\mathbf{P}:\dot{\mathbf{F}}+\bm{\xi}\cdot\dot{c}_{,\mathbf{X}}-\pi\dot{c}.
\end{equation}
The second law of thermodynamics leads to
\begin{equation}
	\Theta\rho_{0}\dot{s}\geq0
\end{equation}
with entropy per unit mass density $s$ and absolute temperature $\Theta$. Note that heat fluxes and heat sources are not considered here. Using $\rho_{0}e=\rho_{0}\Psi+\Theta\rho_{0}s$ this can be rewritten as
\begin{equation} \label{eq:entropy_balance}
	\rho_{0}\dot{\Psi}\leq\rho_{0}\dot{e}
\end{equation}
with Helmholtz free energy $\Psi$. Using \eqref{eq:energy_balance} and \eqref{eq:entropy_balance}, a thermodynamically consistent model is achieved as long as $\Psi$ fulfills
\begin{equation} \label{eq:thermodyn_cons}
	\begin{aligned}
		\rho_{0}\dot{\Psi} &\leq \mathbf{P}:\dot{\mathbf{F}}+\bm{\xi}\cdot\dot{c}_{,\mathbf{X}}-\pi\dot{c} \qquad \text{or} \\
		\rho_{0}\dot{\Psi} &\leq \dfrac{1}{2}\mathbf{S}:\dot{\mathbf{C}}+\bm{\xi}\cdot\dot{c}_{,\mathbf{X}}\pi\dot{c},
	\end{aligned}
\end{equation}
respectively. $\mathbf{S}$ denotes the $2^{nd}$ Piola-Kirchhoff stress tensor and $\mathbf{C}=\mathbf{F}^{T}\mathbf{F}$ the right Cauchy-Green tensor.

Assuming that $\Psi=\Psi\left(\mathbf{C},\mathbf{C}^{p},\mathbf{Q},c,c_{,\mathbf{C}},\dot{c}\right)$ with plastic right Cauchy-Green tensor $\mathbf{C}^{p}={\mathbf{F}^{p}}^{T}\mathbf{F}^{p}$ and set of internal plastic variables $\mathbf{Q}$, \eqref{eq:thermodyn_cons} can be written as
\begin{equation} \label{eq:thermodyn_cons2}
	\begin{aligned}
	\int\limits_{\Omega_{0}}&\left(\rho_{0}\dfrac{\partial\Psi}{\partial\mathbf{C}}:\dot{\mathbf{C}}+\rho_{0}\dfrac{\partial\Psi}{\partial\mathbf{C}^{p}}:\dot{\mathbf{C}}^{p}+\rho_{0}\dfrac{\partial\Psi}{\partial\mathbf{Q}}\cdot\dot{\mathbf{Q}}+\rho_{0}\dfrac{\partial\Psi}{\partial c}\dot{c}+\rho_{0}\dfrac{\partial\Psi}{\partial c_{,\mathbf{X}}}\cdot\dot{c}_{,\mathbf{X}}+\rho_{0}\dfrac{\partial\Psi}{\partial\dot{c}}\ddot{c}\right)\mathrm{d}\Omega_{0} \\
...
The plastic dissipation $\mathbb{D}^{p}=-\rho_{0}\dfrac{\partial\Psi}{\partial\mathbf{C}^{p}}:\dot{\mathbf{C}}^{p}-\rho_{0}\dfrac{\partial\Psi}{\partial\mathbf{Q}}\cdot\dot{\mathbf{Q}}$ can be set to zero for purely elastic deformations. Comparing the terms on the left hand and right hand side of \eqref{eq:thermodyn_cons2}, the following relations can be derived:
\begin{equation} \label{eq:relations_cons}
	\begin{aligned}
		\rho_{0}\dfrac{\partial\Psi}{\partial\dot{c}} &= 0, \\
		\rho_{0}\dfrac{\partial\Psi}{\partial c_{,\mathbf{X}}} &= \bm{\xi}, \\
		2\rho_{0}\dfrac{\partial\Psi}{\partial\mathbf{C}} &= \mathbf{S}, \\
		\left(\pi+\rho_{0}\dfrac{\partial\Psi}{\partial c}\right)\dot{c} &\leq 0, \\
		\mathbb{D}^{p} &\geq 0.
	\end{aligned}
\end{equation}
$\eqref{eq:relations_cons}_{4}$ can be rewritten as
\begin{equation} \label{eq:relation_cons_beta}
	\pi = -\rho_{0}\dfrac{\partial\Psi}{\partial c}-\beta\dot{c}
\end{equation}
with function $\beta\geq0$. Inserting \eqref{eq:relations_cons} and \eqref{eq:relation_cons_beta} into the balance laws $\eqref{eq:lin_mom_ang_mom}_{1}$ and \eqref{eq:microforce_balance}, leads to following equations (using $\mathbf{P}=\mathbf{F}\mathbf{S}$):
\begin{equation} \label{eq:balance_laws}
	\begin{aligned}
		Div\left(2\mathbf{F}\rho_{0}\dfrac{\partial\Psi}{\partial\mathbf{C}}\right)+\mathbf{B} &= \rho_{0}\ddot{\mathbf{U}}, \\
		Div\left(\rho_{0}\dfrac{\partial\Psi}{\partial c_{,\mathbf{X}}}\right)+l-\rho_{0}\dfrac{\partial\Psi}{\partial c} &= \beta\dot{c}.
	\end{aligned}
\end{equation}
...
\begin{equation} \label{eq:Helmholtz_ductile2}
	\rho_{0}\Psi_{n}\left(\mathbf{C},\mathbf{C}^{p},\mathbf{Q},c,c_{,\mathbf{X}}\right) = g\left(c\right)\Psi^{+}\left(\mathbf{C},\mathbf{C}^{p}\right)+\Psi^{-}\left(\mathbf{C},\mathbf{C}^{p}\right)+g_{p}\left(c\right)\Psi_{p}\left(\mathbf{Q}\right)+\Gamma_{c,n}.
\end{equation}
\hl{plrRho0dabei} Using this, the partial derivatives required in \eqref{eq:balance_laws} can be computed. Furthermore, setting $l=0$ and $\beta=0$ leads to the governing equations. The stress equilibrium is given by
...
\begin{equation} \label{eq:c2_equil_ductile}
	\left\{\begin{alignedat}{2}
		...
		\hl{\nabla c\cdot\mathbf{N}} &= 0 && \quad \text{on } \partial\Omega_{0}\times\left(0,T\right) \\
		\mathcal{H} &= \mathcal{H}_{0} && \quad \text{on } \Omega_{0}\times0,
	\end{alignedat}\right.
\end{equation}
\hl{initial condition right?!}

and for the fourth-order phase-field theory
