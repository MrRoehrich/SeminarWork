...
\hl{weak and semidiscrete Galerkin form for ductile fracture}
...
Considering time step $n$, time step size $\Delta t = t_{n+1}-t_{n}$, parameters $\alpha_{f},\alpha_{m},\beta,\gamma$ as well as vectors $\mathbf{d}_{n}$ and $\bm{\Phi}_{n}$ for the control variable degrees-of-freedom of the displacements and phase-field, $\textbf{v}_{n}=\dot{\mathbf{d}}_{n}$ and $\mathbf{a}_{n}=\ddot{\mathbf{d}}_{n}$ are defined. The method then reads as follows:
...
\begin{equation} \label{eq:NewtonRaphson}
	\begin{aligned}
	\begin{alignedat}{2}
		\dfrac{\partial\mathbf{R}_{i}^{u}}{\partial\mathbf{a}_{n+1}}\Delta \mathbf{a} &+ \dfrac{\partial\mathbf{R}_{i}^{u}}{\partial\Phi_{n+1}}\Delta\bm{\Phi} &&= -\mathbf{R}_{i}^{u}, \\
		\dfrac{\partial\mathbf{R}_{i}^{c}}{\partial\mathbf{a}_{n+1}}\Delta \mathbf{a} &+ \dfrac{\partial\mathbf{R}_{i}^{c}}{\partial\Phi_{n+1}}\Delta\bm{\Phi} &&= -\mathbf{R}_{i}^{c},
	\end{alignedat}
	\end{aligned}
\end{equation}
